...
export default function SettingsPage() {
	...
	const totalRetailValue = products.reduce((sum, p) => sum + p.retailPriceUSD * p.stockQuantity, 0);
		...
		<AppShell>
			<div className="space-y-6">
				...
				<div>
					...
					<p className="mt-1 text-muted-foreground">Gestiona la configuracion de tu tienda</p>
				</div>
					...
					<Card>
						...
						<CardContent className="space-y-4">
							<div className="flex items-center justify-between rounded-lg bg-secondary/30 p-4">
								<div>
									...
									<p className="text-xs text-muted-foreground">
										Actualizado {format(new Date(exchangeRate.updatedAt), 'PPp', { locale: es })}{' '}
							...
							<div className="rounded-lg border border-border p-3">
								<p className="text-xs text-muted-foreground">
									El tipo de cambio se aplica automaticamente a todos los precios en ARS. Los
									precios en USD permanecen sin cambios.
								</p>
							</div>
						</CardContent>
					</Card>
					...
					<Card className="lg:col-span-2">
						...
						<CardContent>
							...
							<div className="mt-4 rounded-lg border border-border p-3">
								<p className="text-xs text-muted-foreground">
									Los datos se exportan en formato CSV compatible con Excel. Los respaldos incluyen
